...
There are N jobs in QWT.DSN.SDSN5TSA which need to be customised and submitted in order to create the Tuning Services Repository Database
* DSN5REPO - create the database, and all the objects within it.
* DSN5BND - binds all the packages.
* DSN5RTRP - grants package priviledges to the QWT server.
* DSN5RUDF - creates the UDFs that control who can use QWT.
* DSN5RTTG - grants priviledges to users of QWT.



## Create the Repository Database (DSN5REPO)

Edit and Run Job DSN5REPO to create the SQL Tuning Services Repository Database.

I tend to take a copy (AAA5REPO) of the original job (DSN5REPO) for editing, so that I always have the original source sample to refer to.
...
Edited JOBLIB to remove SDSNEXIT
...
## Bind the QWT Packages (DSN5BND)

Edit and Run Job DSN5BND to bind the SQT packages to the Repository Database. Job DSN5BND is designed for the scenario where the same Db2 z/OS subsystem is use as the target database as well as the Repository Database. Other sample jobs are provided for binding the a subset of packages to Subsystems that are target-only or repository-only.
...
## Grant EXECUTE privileges (DSN5RTRP)


Job QWT.DSN.SDSN5TSA(DSN5RTRP) contains the JCL to grant permissions for SQL processing by Tuning Services. 
...
## Tuning Privileges (DSN5RTTG)



Next, I copied DSN5RTTG into member AAA5RTTG for editing. This job  grant package privileges, privileges to run several specific tuning APIs, and the privilege to create EXPLAIN tables to the intended tuning IDs. I perform the following edits
